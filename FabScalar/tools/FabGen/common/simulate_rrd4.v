...
begin:Processor_Initialization
 ...
 begin
        {reg_part4, reg_part3, reg_part2, reg_part1} = $getArchRegValue(i);
        fabScalar.reg_read.PhyRegFile1.sram[i] = reg_part1;
        fabScalar.reg_read.PhyRegFile2.sram[i] = reg_part2;
        fabScalar.reg_read.PhyRegFile3.sram[i] = reg_part3;
        fabScalar.reg_read.PhyRegFile4.sram[i] = reg_part4;
	LOGICAL_REG[i]			      = $getArchRegValue(i);	
  	$display(" LOGICAL_REG[i]: %h", $getArchRegValue(i));
 end
        {reg_part4, reg_part3, reg_part2, reg_part1} = $getArchRegValue(65);
        fabScalar.reg_read.PhyRegFile1.sram[32] = reg_part1;
        fabScalar.reg_read.PhyRegFile2.sram[32] = reg_part2;
        fabScalar.reg_read.PhyRegFile3.sram[32] = reg_part3;
        fabScalar.reg_read.PhyRegFile4.sram[32] = reg_part4;

        {reg_part4, reg_part3, reg_part2, reg_part1} = $getArchRegValue(64);
        fabScalar.reg_read.PhyRegFile1.sram[33] = reg_part1;
        fabScalar.reg_read.PhyRegFile2.sram[33] = reg_part2;
        fabScalar.reg_read.PhyRegFile3.sram[33] = reg_part3;
        fabScalar.reg_read.PhyRegFile4.sram[33] = reg_part4;
...
begin:REG_READ
 ...
 $fwrite(fd6,"CYCLE=%d\n",sim_count);
 $fwrite(fd6,"----------------------------------------------------------------------\n");
 $fwrite(fd6,"FU0: PRF[34]: %h bypassValid0_i:%b recoverFlag:%b bypass0Dest:%d bypass0Data_stage1i: %h \n", fabScalar.reg_read.PhyRegFile1.sram[34], fabScalar.reg_read.bypassValid0_i, fabScalar.reg_read.recoverFlag_i,fabScalar.reg_read.bypass0Dest, fabScalar.reg_read.bypass0Data_stage1i);
 $fwrite(fd6,"FU0 Valid=%b PC=0x%h ALid: %d Src1=%d Src1Data=0x%h Src2=%d Src2Data=0x%h \n",
	 fabScalar.reg_read.fuPacketValid0_o,
	 fabScalar.reg_read.fuPacket0_i[`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG:`SIZE_PC+`SIZE_CTI_LOG+1],
	 fabScalar.reg_read.fuPacket0_i[`SIZE_ACTIVELIST_LOG+`CHECKPOINTS_LOG+`SIZE_PHYSICAL_LOG+`SIZE_IMMEDIATE+
                        `LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG :`CHECKPOINTS_LOG+`SIZE_PHYSICAL_LOG+`SIZE_IMMEDIATE+
                        `LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG+1],
 ...
 $fwrite(fd6,"FU1 Valid=%b PC=0x%h Src1=%d Src1Data=0x%h Src2=%d Src2Data=0x%h \n",
 ...
 $fwrite(fd6,"FU2 Valid=%b PC=0x%h Src1=%d Src1Data=0x%h Src2=%d Src2Data=0x%h \n",
 ...
 $fwrite(fd6,"FU3 Valid=%b PC=0x%h Src1=%d Src1Data=0x%h Src2=%d Src2Data=0x%h \n",
 ...
 */
 ...
 begin
	...
	$fwrite(fd10,"AGEN PC:%h isLoad:%b isStore:%b addr:%h data:%h size:%b lsq ID:%d ALid:%d BrMask:%b\n",
		...
		$fwrite(fd10,"ldViolationPacket_o: %d",fabScalar.lsu.ldViolationPacket_o);
	end
...
begin:WRITE_BACK
 ...
 $fwrite(fd8,"CYCLE=%d\n",sim_count);
 $fwrite(fd8,"----------------------------------------------------------------------\n");
 $fwrite(fd8,"FU0_l1: ALid: %d WBFlags: %b",fabScalar.writebk.ctrlFU0_l1[`SIZE_ACTIVELIST_LOG+`WRITEBACK_FLAGS-1:`WRITEBACK_FLAGS], fabScalar.writebk.ctrlFU0_l1[`WRITEBACK_FLAGS-1:0]);
 $fwrite(fd8,"FU0_l2: ALid: %d WBFlags: %b",fabScalar.writebk.ctrlFU0_l2[`SIZE_ACTIVELIST_LOG+`WRITEBACK_FLAGS-1:`WRITEBACK_FLAGS], fabScalar.writebk.ctrlFU0_l2[`WRITEBACK_FLAGS-1:0]);
 $fwrite(fd8,"FU0_l3: ALid: %d WBFlags: %b",fabScalar.writebk.ctrlFU0_l3[`SIZE_ACTIVELIST_LOG+`WRITEBACK_FLAGS-1:`WRITEBACK_FLAGS], fabScalar.writebk.ctrlFU0_l3[`WRITEBACK_FLAGS-1:0]);

 $fwrite(fd8,"\nldViolationPacket_i: %d ldViolationPacket_o: %d \n",fabScalar.writebk.ldViolationPacket_i, fabScalar.writebk.ldViolationPacket_o);
 $fwrite(fd8,"FU0: Result:%h WB_Flag:%b\n",
		fabScalar.writebk.bypassPacket0_o[`SIZE_DATA+`CHECKPOINTS_LOG:`CHECKPOINTS_LOG+1],
		fabScalar.writebk.exePacket0Flags);
 $fwrite(fd8,"FU1: WB_Flag:%b\n",
		fabScalar.writebk.exePacket1Flags);
 $fwrite(fd8,"FU2:        Mispredict=%b Conditional Br=%b direction=%b SMT id=%d Ctiq id=%d Target Addr=%h\n",
	...
	/*$fwrite(fd8,"        DestReg:%d Result:%h\n",
		fabScalar.writebk.wb_inst2DestReg,
		fabScalar.writebk.wb_inst2Data);*/
 $fwrite(fd8,"FU3 WB ALid=%d DestReg:%d Result:%h WB flags:%b\n",
...
begin:COMMIT
 ...
 reg_part1      = fabScalar.reg_read.PhyRegFile1.sram[fabScalar.activeList.dataAl0[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part2      = fabScalar.reg_read.PhyRegFile2.sram[fabScalar.activeList.dataAl0[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part3      = fabScalar.reg_read.PhyRegFile3.sram[fabScalar.activeList.dataAl0[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part4      = fabScalar.reg_read.PhyRegFile4.sram[fabScalar.activeList.dataAl0[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 result[0]      = {reg_part4, reg_part3, reg_part2, reg_part1};

 reg_part1      = fabScalar.reg_read.PhyRegFile1.sram[fabScalar.activeList.dataAl1[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part2      = fabScalar.reg_read.PhyRegFile2.sram[fabScalar.activeList.dataAl1[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part3      = fabScalar.reg_read.PhyRegFile3.sram[fabScalar.activeList.dataAl1[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part4      = fabScalar.reg_read.PhyRegFile4.sram[fabScalar.activeList.dataAl1[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 result[1]      = {reg_part4, reg_part3, reg_part2, reg_part1};

 reg_part1      = fabScalar.reg_read.PhyRegFile1.sram[fabScalar.activeList.dataAl2[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part2      = fabScalar.reg_read.PhyRegFile2.sram[fabScalar.activeList.dataAl2[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part3      = fabScalar.reg_read.PhyRegFile3.sram[fabScalar.activeList.dataAl2[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part4      = fabScalar.reg_read.PhyRegFile4.sram[fabScalar.activeList.dataAl2[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 result[2]      = {reg_part4, reg_part3, reg_part2, reg_part1};

 reg_part1      = fabScalar.reg_read.PhyRegFile1.sram[fabScalar.activeList.dataAl3[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part2      = fabScalar.reg_read.PhyRegFile2.sram[fabScalar.activeList.dataAl3[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part3      = fabScalar.reg_read.PhyRegFile3.sram[fabScalar.activeList.dataAl3[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 reg_part4      = fabScalar.reg_read.PhyRegFile4.sram[fabScalar.activeList.dataAl3[2*`SIZE_PHYSICAL_LOG:`SIZE_PHYSICAL_LOG+1]];
 result[3]      = {reg_part4, reg_part3, reg_part2, reg_part1};
 ...
 begin
	//$fwrite(fd7,"LD Violation Occured -> PC:%h\n",fabScalar.activeList.recoverPC);
		...
		reg_part1      = fabScalar.reg_read.PhyRegFile1.sram[fabScalar.amt.AMT.sram[j]];
		reg_part2      = fabScalar.reg_read.PhyRegFile2.sram[fabScalar.amt.AMT.sram[j]];
		reg_part3      = fabScalar.reg_read.PhyRegFile3.sram[fabScalar.amt.AMT.sram[j]];
		reg_part4      = fabScalar.reg_read.PhyRegFile4.sram[fabScalar.amt.AMT.sram[j]];

		if({reg_part4, reg_part3, reg_part2, reg_part1} != LOGICAL_REG[j])
		$display("Real[%d]:%h  Virt[%d]:%h",j,{reg_part4, reg_part3, reg_part2, reg_part1},j,LOGICAL_REG[j]);
        end
 ...
 if(sim_count % 5000 == 0) 
 begin
	if((fabScalar.activeList.commitCount-last_commit_cnt) < 100)
	begin
 ...
 $fwrite(fd7,"CYCLE=%d\n",sim_count);
 $fwrite(fd7,"----------------------------------------------------------------------\n");
 $fwrite(fd7,"ctrlAl0: %b fuAddr0:%d fuData0: %b \n", fabScalar.activeList.ctrlAl0, fabScalar.activeList.fuAddr0, fabScalar.activeList.fuData0);
 $fwrite(fd7,"ldViolationPacket_i: %d\n",fabScalar.activeList.ldViolationPacket_i);
 ...
 $fwrite(fd7,"ctrlAl0: %b\n",fabScalar.activeList.ctrlAl0);
 if(fabScalar.activeList.recoverFlag)
 begin
        $fwrite(fd7,"LD Violation Occured -> PC:%h\n",fabScalar.activeList.recoverPC);
 end
 ...
 begin
	for(i=0;i<`SIZE_RMT;i=i+1)
        $fwrite(fd7,"PRF[%d]:%h  LOGICAL_REG[%d]:%h\n",i,fabScalar.reg_read.PhyRegFile.sram[i],i,
		LOGICAL_REG[i]);
 end
 ...
 */
 ...
 $fwrite(fd7,"Commit Cnt=%d: PC:%h LogicDest=%d PhyDest=%d Value=%h FreedPhyReg=%d isStore=%b isLoad=%b isFission=%b CTI:%b\n", 
	 ...
	 {reg_part4,reg_part3,reg_part2,reg_part1},
...
begin:HANDLE_EXCEPTION
 ...
 begin
	$display("CYCLE:%d Exception is High\n",sim_count);
        for(j=0;j<`SIZE_RMT;j=j+1)
        begin
                {reg_part4, reg_part3, reg_part2, reg_part1}  = LOGICAL_REG[j];
                fabScalar.reg_read.PhyRegFile1.sram[j] = reg_part1;
                fabScalar.reg_read.PhyRegFile2.sram[j] = reg_part2;
                fabScalar.reg_read.PhyRegFile3.sram[j] = reg_part3;
                fabScalar.reg_read.PhyRegFile4.sram[j] = reg_part4;
        end
 end
end
