...
begin:Processor_Initialization
 ...
 begin
        {reg_part2, reg_part1} = $getArchRegValue(i);
...
begin:COMMIT
 ...
 begin
	//$fwrite(fd7,"LD Violation Occured -> PC:%h\n",fabScalar.activeList.recoverPC);
		...
		if({reg_part2, reg_part1} != LOGICAL_REG[j])
		$display("Real[%d]:%h  Virt[%d]:%h",j,{reg_part2, reg_part1},j,LOGICAL_REG[j]);
        end
...
begin:HANDLE_EXCEPTION
 ...
 begin
	$display("CYCLE:%d Exception is High\n",sim_count);
        for(j=0;j<`SIZE_RMT;j=j+1)
        begin
                {reg_part2, reg_part1}  = LOGICAL_REG[j];
