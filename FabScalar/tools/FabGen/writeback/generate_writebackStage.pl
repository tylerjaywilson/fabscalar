...
#
# Purpose: This script creates all verilog files in the folder writeback.
################################################################################
...
{
	print "\n";
	print "Usage: perl $scriptName -n A B C D -d <pipe_depth> [-m] [-v] [-h]\n";
...
if(-d "$PATH/$stage")
{
	`rm -f $PATH/$stage/WriteBack.v`;
}

`perl generate_WB.pl -w $issueWidth -p $depth -dw 32 > $PATH/$stage/WriteBack.v`; # Tanmay's script
#`perl generate_WriteBack.pl -n $nString $mString > $PATH/$stage/WriteBack.v`; # My script
