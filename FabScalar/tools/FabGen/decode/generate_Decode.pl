...
#
# Purpose: This script creates Decode.v file.
################################################################################
...
{
	print "\n";
	print "Usage: perl $scriptName -dw <decode_width> -dd <decode_depth> [-ba] [-m] [-v] [-h]\n";
	print "\t-m: Add header\n";
	print "\t-ba: Produce Decode to Support Block Ahead Fetch\n";
...
# Create module name
$outputFileName = "Decode.v";
$moduleName = "Decode";

# Quit if decode width and depth is not supported
if($supportBA)
{
	$copyFileName = "Decode_w${decodeWidth}_d${decodeDepth}_ba.v";	
}
else
{
	$copyFileName = "Decode_w${decodeWidth}_d${decodeDepth}.v";
}
