...
int readSignedByte_calltf(char *user_data)
{
  int data;
  ...
  data = (int) VMEM[0]->read_signed_byte(current_value.value.integer);
...
void readSignedByte_register() {
  s_vpi_systf_data task_data_s;

  task_data_s.type      = vpiSysFunc;
  //task_data_s.sysfunctype = vpiSysFuncSized;
  task_data_s.tfname    = "$readSignedByte";
  task_data_s.calltf    = readSignedByte_calltf;
